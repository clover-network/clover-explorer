...
var vecScaleTo = function vecScaleTo(v, length) {
  ...
  return _matrixUtil.vec2.scale([], _matrixUtil.vec2.normalize([], v), length);
};
...
function roundedHull(polyPoints, padding) {
  ...
  var roundedHull2 = function roundedHull2(points) {
    var offsetVector = _matrixUtil.vec2.scale([], unitNormal(points[0], points[1]), padding);

    var invOffsetVector = _matrixUtil.vec2.scale([], offsetVector, -1);

    var p0 = _matrixUtil.vec2.add([], points[0], offsetVector);

    var p1 = _matrixUtil.vec2.add([], points[1], offsetVector);

    var p2 = _matrixUtil.vec2.add([], points[1], invOffsetVector);

    var p3 = _matrixUtil.vec2.add([], points[0], invOffsetVector);
  ...
  for (var segmentIndex = 0; segmentIndex < segments.length; ++segmentIndex) {
    ...
    var offset = _matrixUtil.vec2.scale([], unitNormal(p0, p1), padding);

    segments[segmentIndex] = [_matrixUtil.vec2.add([], p0, offset), _matrixUtil.vec2.add([], p1, offset)];
  }
...
function paddedHull(polyPoints, padding) {
  ...
  var smoothHull2 = function smoothHull2(points) {
    ...
    var extension0 = _matrixUtil.vec2.add([], points[0], _matrixUtil.vec2.scale([], extensionVec, -1));

    var extension1 = _matrixUtil.vec2.add([], points[1], extensionVec);

    var tangentHalfLength = 1.2 * padding;
    var controlDelta = vecScaleTo(_matrixUtil.vec2.normalize([], v), tangentHalfLength);

    var invControlDelta = _matrixUtil.vec2.scale([], controlDelta, -1);

    var control0 = _matrixUtil.vec2.add([], extension0, invControlDelta);

    var control1 = _matrixUtil.vec2.add([], extension1, invControlDelta);

    var control3 = _matrixUtil.vec2.add([], extension0, controlDelta);
  ...
  var hullPoints = polyPoints.map(function (point, index) {
    ...
    return {
      p: point,
      v: _matrixUtil.vec2.normalize([], vecFrom(point, pNext))
    };
  ...
  for (var i = 0; i < hullPoints.length; ++i) {
    ...
    var extensionVec = _matrixUtil.vec2.normalize([], _matrixUtil.vec2.add([], hullPoints[priorIndex].v, _matrixUtil.vec2.scale([], hullPoints[i].v, -1)));

    hullPoints[i].p = _matrixUtil.vec2.add([], hullPoints[i].p, _matrixUtil.vec2.scale([], extensionVec, padding));
  }
